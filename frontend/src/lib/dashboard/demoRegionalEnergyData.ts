export type RegionalEnergyType =
  | "renewable_potential"
  | "conventional_energy_hub"
  | "industrial_demand"
  | "high_demand"
  | "remote_energy_need"
  | "hybrid_production_demand";

export type RegionalRiskLevel = "Low" | "Medium" | "Medium-High" | "High" | "Critical";
...
export interface RegionalEnergyData {
  ...
  lat: number;
  lng: number;
  type: RegionalEnergyType;
  energyRole: string;
  currentLoadMW: number;
  solarPotential: number;
  windPotential: number;
  industrialDemandScore: number;
  ...
  humidity: number;
  dataQuality: number;
  bestEnergyStrategy: string;
  recommendation: string;
}

export const demoRegionalEnergyData: RegionalEnergyData[] = [
...
export function getRegionalSummary(regions = demoRegionalEnergyData) {
  const highestSolar = maxBy(regions, "solarPotential");
  const highestDemand = maxBy(regions, "demandPressure");
  const highestRisk = [...regions].sort((a, b) => riskWeight(b.riskLevel) - riskWeight(a.riskLevel) || a.gridHealth - b.gridHealth)[0];
  const bestHybrid = regions.filter((region) => region.type === "hybrid_production_demand").sort((a, b) => b.windPotential + b.solarPotential - (a.windPotential + a.solarPotential))[0];
  return { highestSolar, highestDemand, highestRisk, bestHybrid };
}

export function riskWeight(risk: RegionalRiskLevel) {
  const weights: Record<RegionalRiskLevel, number> = { Low: 1, Medium: 2, "Medium-High": 3, High: 4, Critical: 5 };
  return weights[risk];
}

function maxBy(items: RegionalEnergyData[], key: "solarPotential" | "demandPressure") {
